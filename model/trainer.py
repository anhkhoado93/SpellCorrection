...
from params import DEVICE, PKL_PATH, MODEL_SAVE_PATH
...
class ModelTrainer:
    def __init__(self, model_name, n_samples,
        ...
        self.device = DEVICE
        self.token_ids = load_file_picke(PKL_PATH+f'{model_name}.pkl')
    ...
    def make_loader(self):
        ...
        if not self.is_split_indexs:
            save_picke_file(PKL_PATH+'train_indexs.pkl', self.train_idxs)
            print("Save done")
            save_picke_file(PKL_PATH+'val_indexs.pkl', self.val_idxs)
            print("Save done")
    ...
    def save_model(self, epoch: int, history: dict):
        ...
        torch.save(model_states, MODEL_SAVE_PATH + f"model/model{epoch}.pth")
        print(f"Save model {epoch} done")
        with open(MODEL_SAVE_PATH + f"history/history{epoch}.pkl", "wb") as file:
            pickle.dump(history, file)
        if epoch > 2:
            os.remove(MODEL_SAVE_PATH + f"model/model{epoch-2}.pth")
